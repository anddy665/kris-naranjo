...
class Frontend extends Base_App {
	...
	protected function init_filters() {
		$responsive_templates_filter = 'breakpoints/get_stylesheet_template';
		...
		add_filter( 'wp_calculate_image_srcset', array( $this, 'fix_elementor_custom_image_srcset' ), 10, 5 );
	}
	...
	public static function widgets_responsive_styles_files_names(): array {
		return array(
			...
			'widget-cmsmasters-split-button',
	...
	/**
	 * Fix Elementor custom image sizes in srcset.
	 *
	 * @since 1.24.0
	 *
	 ...
	 */
	public function fix_elementor_custom_image_srcset( $sources, $size_array, $image_src, $image_meta, $attachment_id ) {
		if ( empty( $image_meta['file'] ) || empty( $sources ) ) {
			return $sources;
		}

		$image_subdir = _wp_get_attachment_relative_path( $image_meta['file'] );

		if ( empty( $image_subdir ) ) {
			return $sources;
		}

		$upload_dir = wp_get_upload_dir();
		$baseurl = trailingslashit( $upload_dir['baseurl'] );
		$elementor_thumbs_dir = defined( 'BFITHUMB_UPLOAD_DIR' ) ? BFITHUMB_UPLOAD_DIR : 'elementor/thumbs';

		foreach ( $sources as $width => $source ) {
			if ( false !== strpos( $source['url'], trailingslashit( $image_subdir ) . $elementor_thumbs_dir . '/' ) ) {
				$sources[ $width ]['url'] = $baseurl . $elementor_thumbs_dir . '/' . basename( $source['url'] );
			}
		}
	...
	public function get_builder_template_content( $post_id, $with_css = false, $with_loop_dynamic_css = false ) {
		...
		$is_build_with_elementor = $elementor->documents->get( $post_id )->is_built_with_elementor();
	...
	public function get_enqueued_template_css( $post_id ) {
		...
		if ( $this->template_document->is_autosave() ) {
			...
			$css_file = PostCSS::create( $this->template_document->get_post()->ID );
		}
